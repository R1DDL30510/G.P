# ======================================================================
# GARVIS v001 – Automated Validation (patched)
#
# This script performs a non‑destructive audit of a running GARVIS stack.
# It checks for the presence of required paths, the availability of the
# Ollama CLI, the listening state of expected ports, basic HTTP health
# of Ollama endpoints and evaluator proxy, and basic YAML sanity.
# Results are emitted to the console and to a JSON report file.
#
# Usage examples:
#   powershell -ExecutionPolicy Bypass -File .\garvis_validate_fixed.ps1
#   powershell -ExecutionPolicy Bypass -File .\garvis_validate_fixed.ps1 -BaseDir "D:\GARVIS" -RouterYaml "D:\GARVIS\router\router.yaml"
#
# This version fixes the following issues from the original:
#   * Replaces fancy en‑dash with a plain hyphen to avoid parse errors.
#   * Ensures curly braces are properly balanced.
#   * Removes mandatory check for a non‑existent 'services' directory.
# ======================================================================
...
function Add-Result {
  param([System.Collections.ArrayList]$ArrRef, $Item)
...
# Check Ollama presence and version
...
$portStatus = if ($missingPorts.Count -eq 0) { 'pass' } else { 'fail' }
...
# Check HTTP health of endpoints
$endpointsToCheck = @(
...
$httpStatus = if ($httpBad.Count -eq 0) { 'pass' } else { 'warn' }
...
if (-not $yamlExists) {
  ...
  if (-not $yamlParsed.ok) {
    ...
    $statusCfg = if ($issues.Count -eq 0) { 'pass' } else { 'fail' }
...
if (-not $QuietJSON) {
  ...
  foreach ($r in $results) {
    ...
    Write-Host ([string]::Format("[{0}] {1}", $r.status.ToUpper(), $r.name)) -ForegroundColor $color
    if ($r.remediation) { Write-Host ("  ↳ Fix: {0}" -f $r.remediation) -ForegroundColor DarkGray }
  }
  Write-Host ''
  Write-Host ([string]::Format('Summary: PASS={0}  WARN={1}  FAIL={2}', $summary.pass, $summary.warn, $summary.fail)) -ForegroundColor Cyan
  Write-Host ([string]::Format('JSON report: {0}', $reportPath)) -ForegroundColor Gray
} else {
