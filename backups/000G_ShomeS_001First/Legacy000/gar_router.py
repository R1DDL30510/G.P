...
def evaluate_choice(prompt: str, hint_model: str | None = None) -> dict:
    """Select the best model alias and endpoint based on prompt, inventory, and policy."""
    ptoks  = estimate_tokens(prompt)
    margin = float(POLICY.get("min_ctx_margin", 0.2))

    candidates = []
    if hint_model and hint_model in INVENTORY:
        meta = INVENTORY[hint_model]
        candidates.append((hint_model, meta["endpoint"], meta))
    else:
        p = (prompt or "").lower()
        prefer = []
